...
const Nav = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Use window.innerHeight as fallback if element not found
      const effectiveHeroBottom = heroBottom > 0 ? heroBottom : window.innerHeight;
      setIsInHero(scrollPosition < effectiveHeroBottom);

      // BUG FIX 2: If in hero section, show navbar immediately
      if (scrollPosition < effectiveHeroBottom) {
        setIsVisible(true);
      } else if (!hoveringTop) {
  ...
  return (
    <nav
      className={`flex justify-center items-center h-[50px] sm:h-[70px] w-full bg-black border-x-2 border-b-2 border-white text-white font-semibold z-[9999] rounded-b-md hover-target 
      transition-transform duration-300 ${isVisible ? "translate-y-0" : "-translate-y-full "
        }`}
    >
      <div className="flex justify-between w-full text-[12px] sm:text-[20px] h-full">
        {["Home", "About", "Projects", "Testimonials", "Contact"].map(
          (item, index, arr) => (
            <div key={index} className={`flex-1 flex justify-center items-center h-full ${index !== arr.length - 1 ? "border-r-2 border-white" : ""}`} >
              <a
                href={`#${item}`}
                className="hover:scale-105 hover:bg-white hover:text-black w-full h-full flex justify-center items-center transition duration-300"
              >
                {item}
              </a>
            </div>
